...
/**
 * @details
 * CLI function that tests I2C communication to fireflies
 */
...
bool firefly_i2ctest_ioexpander_helper(char* m) {
...
//TODO left off breaking up tests

/**
 * @details
 * CLI function that tests I2C communication to fireflies/IO expanders
 */
BaseType_t firefly_i2ctest_ctl(int argc, char **argv, char *m) {
  ...
  if (!firefly_i2ctest_ioexpander_helper(m)) {
    return pdFALSE;
  }

  //if (!firefly_i2ctest_muxreset_helper(m)) {
  //  return pdFALSE;
  //}
