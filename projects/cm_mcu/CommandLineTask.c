...
static BaseType_t i2c_ctl_set_dev(int argc, char **argv) {
  ...
  if (status == 0) {
    ...
    snprintf(m, s,
        "Invalid i2c device %d (%s), only 1,2,3, 4 and 6 supported\r\n", i,
        argv[1]);
  }
  else if (status == -2) {
...
static BaseType_t i2c_ctl_reg_r(int argc, char **argv) {
  ...
  if (status == 0) {
    snprintf(m, s,
        "i2cr: add: 0x%02x, reg 0x%02x: value 0x%02x %02x %02x %02x\r\n",
...
static BaseType_t i2c_ctl_reg_w(int argc, char **argv) {
  ...
  if (status == 0) {
    snprintf(m, s,
        "%s: Wrote to address 0x%x, register 0x%x, value 0x%08x (%d bytes)\r\n",
        argv[0], address, reg_address, packed_data, nbytes - 1);
  }
  else if (status == -1) {
    snprintf(m, s, "%s: operation failed (1)\r\n", argv[0]);
  }
...
static BaseType_t psmon_ctl(int argc, char **argv) {
  ...
  for (int ps = 0; ps < dcdc_args.n_devices; ++ps) {
    ...
    for (int page = 0; page < dcdc_args.n_pages; ++page) {
      float val = dcdc_args.pm_values[ps
          * (dcdc_args.n_commands * dcdc_args.n_pages)
          + page * dcdc_args.n_commands + i1];
      int tens, frac;
      float_to_ints(val, &tens, &frac);
      copied += snprintf(m + copied, SCRATCH_SIZE - copied, "VALUE %02d.%02d\t",
          tens, frac);
    }
    copied += snprintf(m + copied, SCRATCH_SIZE - copied, "\r\n");
  }
...
static BaseType_t clock_ctl(int argc, char **argv) {
  ...
  if (status == -1)
    copied += snprintf(m + copied, SCRATCH_SIZE - copied,
        "%s operation failed (1). \r\n", argv[0]);
  else if (status == -2)
    copied += snprintf(m + copied, SCRATCH_SIZE - copied,
        "%s operation failed (2). \r\n", argv[0]);
  else if (status != 0)
    copied += snprintf(m + copied, SCRATCH_SIZE - copied,
        "%s invalid return value. \r\n", argv[0]);
  return pdFALSE;
}
  ...
  if (argc == 1) { // default command: temps
    ...
    for (; whichff < NFIREFLIES; ++whichff) {
      ...
      if ((1 << whichff) & ff_config) // val > 0 )
        copied += snprintf(m + copied, SCRATCH_SIZE - copied, "%17s: %2d", name,
            val);
      else
        // dummy value
    ...
    if (strncmp(argv[argc - 1], "all", 3) == 0) {
      ...
      if (whichFF >= NFIREFLIES
          || (whichFF == 0 && strncmp(argv[argc - 1], "0", 1) != 0)) {
        snprintf(m + copied, SCRATCH_SIZE - copied,
    ...
    if (argc == 4) { // command + three arguments
      ...
      uint8_t code;
      uint32_t data = (whichFF & FF_MESSAGE_CODE_REG_FF_MASK)
          << FF_MESSAGE_CODE_REG_FF_OFFSET;
      ;;
      ...
      // Add here
        ...
        if (whichFF == NFIREFLIES) {
          copied += snprintf(m + copied, SCRATCH_SIZE - copied,
              "%s: cannot read all registers\r\n", argv[0]);
          return pdFALSE;
        }
        // register number
        uint8_t regnum = strtol(argv[2], NULL, 16);
        copied += snprintf(m + copied, SCRATCH_SIZE - copied,
            "%s: reading FF %s, register 0x%x\r\n", argv[0], getFFname(whichFF),
            regnum);
        // pack channel and register into the data
        data |= ((regnum & FF_MESSAGE_CODE_REG_REG_MASK)
            << FF_MESSAGE_CODE_REG_REG_OFFSET);
      }
      else {
        snprintf(m + copied, SCRATCH_SIZE - copied,
            "%s: command %s not recognized\r\n", argv[0], argv[1]);
        return pdFALSE;
      }
      uint32_t message = ((code & FF_MESSAGE_CODE_MASK)
          << FF_MESSAGE_CODE_OFFSET) | (data & FF_MESSAGE_DATA_MASK);
      xQueueSendToBack(xFFlyQueueIn, &message, pdMS_TO_TICKS(10));
      snprintf(m + copied, SCRATCH_SIZE - copied, "%s: command %s %s sent.\r\n",
          argv[0], argv[1], argv[2]);
      if (receiveAnswer) {
        BaseType_t f = xQueueReceive(xFFlyQueueOut, &message,
            pdMS_TO_TICKS(500));
        if (f == pdTRUE)
          snprintf(m + copied, SCRATCH_SIZE - copied,
              "%s: Command returned 0x%x.\r\n", argv[0], message & 0xFFU);
        else
      ...
      if (strncmp(argv[1], "regw", 4) == 0) {
        ...
        uint32_t data = ((regnum & FF_MESSAGE_CODE_REG_REG_MASK)
            << FF_MESSAGE_CODE_REG_REG_OFFSET)
            | ((value & FF_MESSAGE_CODE_REG_DAT_MASK)
                << FF_MESSAGE_CODE_REG_DAT_OFFSET)
            | ((channel & FF_MESSAGE_CODE_REG_FF_MASK)
                << FF_MESSAGE_CODE_REG_FF_OFFSET);
        uint32_t message = ((code & FF_MESSAGE_CODE_MASK)
            << FF_MESSAGE_CODE_OFFSET)
...
void snapdump(struct dev_i2c_addr_t *add, uint8_t page, uint8_t snapshot[32],
              bool reset);
...
static BaseType_t snapshot(int argc, char **argv) {
  ...
  if (page < 0 || page > 1) {
    copied += snprintf(m + copied, SCRATCH_SIZE - copied,
        "%s: page %d must be between 0-1\r\n", argv[0], page);
    return pdFALSE;
  }
  if (which < 0 || which > 4) {
    copied += snprintf(m + copied, SCRATCH_SIZE - copied,
        "%s: device %d must be between 0-4\r\n", argv[0], which);
    return pdFALSE;
  }
  copied += snprintf(m + copied, SCRATCH_SIZE - copied,
      "%s: page %d of device %s\r\n", argv[0], page, pm_addrs_dcdc[which].name);

  bool reset = false;
  int ireset = strtol(argv[2], NULL, 10);
  if (ireset == 1) reset = true;
...
static BaseType_t sensor_summary(int argc, char **argv) {
  ...
  for (int ps = 0; ps < dcdc_args.n_devices; ++ps) {
    for (int page = 0; page < dcdc_args.n_pages; ++page) {
      float thistemp = dcdc_args.pm_values[ps
          * (dcdc_args.n_commands * dcdc_args.n_pages)
          + page * dcdc_args.n_commands + 0];
      if (thistemp > max_temp) max_temp = thistemp;
    }
  }
...
static BaseType_t eeprom_info(int argc, char **argv) {
  ...
  copied += snprintf(m + copied, SCRATCH_SIZE - copied,
      "EEPROM has 96 blocks of 64 bytes each.\r\n");
  copied += snprintf(m + copied, SCRATCH_SIZE - copied,
      "Block 0 \t 0x0000-0x0040 \t Free.\r\n");
  copied +=
      snprintf(m + copied, SCRATCH_SIZE - copied,
          "Block 1 \t 0x0040-0x007c \t Apollo ID Information. Password: 0x12345678\r\n");
  copied += snprintf(m + copied, SCRATCH_SIZE - copied,
      "Blocks %u-%u \t 0x%04x-0x%04x \t Error buffer.\r\n", EBUF_MINBLK,
      EBUF_MAXBLK, EEPROMAddrFromBlock(EBUF_MINBLK),
      EEPROMAddrFromBlock(EBUF_MAXBLK + 1) - 4);
...
static BaseType_t errbuff_in(int argc, char **argv) {
  ...
  copied += snprintf(m + copied, SCRATCH_SIZE - copied,
      "Data written to EEPROM buffer: %x\r\n", data);
...
static BaseType_t errbuff_out(int argc, char **argv) {
  ...
  if (num > max_entries) {
    copied += snprintf(m + copied, SCRATCH_SIZE - copied,
        "Please enter a number 64 or less \r\n");
    return pdFALSE;
  }
  ...
  for (; i < num; ++i) {
    ...
    if (errcode == EBUF_CONTINUATION && i != 0) {
      ...
      copied += errbuffer_get_messagestr(word, m + copied,
          SCRATCH_SIZE - copied);
    }
  ...
  if (pxTaskStatusArray != NULL) {
    ...
    if (ulTotalRunTime > 0) {
      ...
      for (x = 0; x < uxArraySize; x++) {
        ...
        uint32_t ulStatsAsPercentage = pxTaskStatusArray[x].ulRunTimeCounter
            / ulTotalRunTime;
...
void vCommandLineTask(void *pvParameters) {
  ...
  struct microrl_user_data_t rl_userdata = { .uart_base = uart_base, };

  struct microrl_config rl_config = { .print = U4Print, // default to front panel
      // set callback for execute
      .execute = execute, .prompt_str = "% ", .prompt_length = 2, .userdata =
          &rl_userdata, };
