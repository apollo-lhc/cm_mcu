#% %
import yaml from pprint import pprint import xml.etree.ElementTree as ET

#% %
    verbose = False

#% %
    with open('../data/zynqmon_2.yml') as f:y = yaml.load(f, Loader = yaml.FullLoader) if (verbose) :pprint(y)

#% %
                                                                 def makeNode(parent:ET.Element, id:str, c:dict, addr2: int, parent_id:str)->ET.Element:node = ET.SubElement(parent, 'node') id = id.replace(' ', '_') start = c['start'] count = c['count'] node.set('id', id)
#address is half of the sensor address since these are 32 bit addresses
                                                                                                                                                                                                                                                                          addr = int(addr2 / 2) remain = addr2 % 2 node.set('address', str(hex(addr)))
#this appears to be on all the nodes
                                                                                                                                                                                                                                                                                                                                           node.set("permission", "r")
#set parameter based on the type
                                                                                                                                                                                                                                                                                                                                                        if (c['type'] == 'int8') :width = 8 format = "d" elif (c['type'] == 'int16') :width = 16 format = "d" elif (c['type'] == 'fp16') :width = 16 format = "fp16" elif (c['type'] == 'char') :node.set('mode', "incremental") format = "c" elif (c['type'] == 'uint32_t') :width = 32 format = "u" elif (c['type'] == 'uint16_t') :width = 16 format = "u" else :print("ERROR: unknown type", c['type']) return None if 'size' in c: #if there is a size, it cannot have a mask node.set('size', str(hex(c['size']))) else :mask =(1 << width) - 1 if (remain == 0) :node.set('mask', "0x{0:08X}".format(mask)) else :node.set('mask', "0x{0:08X}".format(mask << 16)) if 'extra' in c:extra = c['extra'] if not"Column" in extra:extra = extra + ";Column=" + id #this semicolon asks for no semicolon in extra when postfixes are included if not"Row" in extra: if parent_id != "" :extra = "Row=" + parent_id + ";" + extra else :extra = "Row=" + id + ";" + extra else :extra = "" node.set('parameters', "Format=" + format + ";" + extra) return node

#% %
#This is the parent(root) tag
#onto which other tags would be
#created
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     cm = ET.Element('node') cm.set('id', 'CM') cm.set('address', '0x00000000')

#% %
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           config = y['config']

                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       for c in config: #loop over entries in configuration(sensor category) i = 0 #counter over the number of sensors within a category names = c['names'] for n in names: #loop over names of sensors within a category if 'postfixes' in c:pp = node = ET.SubElement(cm, 'node') pp.set('id', n) start = c['start'] addr = int((start + i) / 2)
#addr = int((start + i) / 2)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      pp.set('address', str(hex(addr)))
#pp.set("mask", "0x{0:08X}".format(0xFFFFFFFF))
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 postfixes = c['postfixes'] j = 0 for p in postfixes: if p == 'RESERVED' :i += 1 j += 1 continue if verbose:print("adding postfix", p, "to node", n) node = makeNode(pp, p, c, j, n) i += 1 j += 1 else :start = c['start'] makeNode(cm, n, c, start + i, "") i += 1
