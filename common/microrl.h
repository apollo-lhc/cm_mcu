...
struct microrl_config {
   ...
   * received)
   */
  const char **(*get_completion)(void *userdata, int argc, const char *const *argv);
#endif
  /**
...
void microrl_set_prompt(microrl_t *pThis, const char *prompt_str, uint8_t prompt_length);
#if MICRORL_USE_COMPLETE
...
void microrl_set_complete_callback(microrl_t *pThis,
                                   const char **(*get_completion)(void *, int,
                                                                  const char *const *));
#endif
...
// string
void microrl_set_execute_callback(microrl_t *pThis, int (*execute)(void *, int, char **));

// set callback for Ctrl+C terminal signal
#if MICRORL_USE_CTRL_C
void microrl_set_sigint_callback(microrl_t *pThis, void (*sigint)(void *userdata));
#endif
